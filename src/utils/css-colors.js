/** CSS named colors from https://www.w3schools.com/cssref/css_colors.php */
...
const BY_LOWER = new Map(
  CSS_COLORS.map((c) => [c.name.toLowerCase(), c])
);
...
export function resolveColorName(raw) {
  ...
  // Keep hex / unknown as-is for round-trip
  return value;
}

export function colorPreviewStyle(name) {
  if (!name || name === "none") {
    return "background: repeating-conic-gradient(#ccc 0% 25%, #fff 0% 50%) 50% / 8px 8px; border: 1px solid #999;";
  }
  const known = getCssColor(name);
  const bg = known ? known.hex : name;
